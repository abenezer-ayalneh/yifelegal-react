import {Chip, Grid, GridProps, Typography, useTheme} from "@mui/material";
...
const Highlight = ({image, type, category, deal, location, date, ...parentProps}: HighlightPropTypes & GridProps) => {
    ...
    return (
        <Grid item xs={12} sm={6} md={3} {...parentProps}>
                            ...
                            <Grid container spacing={1} padding={2} direction={"column"}>
                                ...
                                <Grid item>
                                    <Chip variant={"outlined"} size={"small"}
                                          label={<Typography sx={{textTransform: "capitalize"}} variant={"body2"}>For {deal}</Typography>} color={deal === "sale" ? "success" : "info"}/>
                                </Grid>
