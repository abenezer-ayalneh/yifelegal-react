...
import GuestHouse from "../../../assets/images/whole-building.jpg"
...
const GuestHouseMyRequestsPage = () => {
    const {t} = useTranslation()
    const params = useParams<DispatcherPageParams>()
    const [selectedGuestHouseDetail, setSelectedGuestHouseDetail] = useState<RequestDetailType[] | null>(null)
    ...
    const handleModalClose = () => setSelectedGuestHouseDetail(null)

    const handleHighlightClicked = (request: RequestDetailType[]) => setSelectedGuestHouseDetail(request)
    ...
    if (!myRequests) {
        ...
        return (
            <>
                ...
                {
                    selectedGuestHouseDetail &&
                    <CustomModal open={Boolean(selectedGuestHouseDetail)} title={"Request Detail"} handleClose={handleModalClose}>
                        <Grid container direction="column" rowSpacing={1} paddingX={{xs: 1, sm: 2, md: 3, lg: 4}} paddingY={2}>
                            {
                                selectedGuestHouseDetail.map((request) => (
...
export default GuestHouseMyRequestsPage