...
const RequestsPage = () => {
  ...
  const { responseData, isRequestLoading } = useFetch(
    {
      method: "GET",
      url: ROOT_URL + `request`,
    },
    "fetch-my-house-requests-on-house-my-request-page"
  );

  const { sendRequest: pay, isRequestLoading: isPaymentBeingProcessed } =
    useSend({
      method: "POST",
      url: ROOT_URL + "payment/pay",
    });

  const requests = useMemo<RequestType[]>(
      ...
      <>
          ...
          {isRequestLoading ? (
            ...
            requests.map((request: RequestType) => (
              <Highlight
                ...
                type={
                  request.detail?.find(
                    (detail) => detail.attribute === "entity"
                  )?.value
                }
        ...
        {selectedRequestDetail && (
          <CustomModal
            ...
            buttons={[
              <LoadingButton
                loading={isPaymentBeingProcessed}
                variant={"contained"}
                color={"primary"}
                fullWidth
