...
const OffRoadForRentSchema = z.object({
...
type OffRoadForRentType = z.infer<typeof OffRoadForRentSchema>;

const CarOffRoadForRent = () => {
    ...
    const {
        ...
        handleSubmit,
        control,
    } = useForm<OffRoadForRentType>({
        resolver: zodResolver(OffRoadForRentSchema),
    ...
    const onSubmitHandler: SubmitHandler<OffRoadForRentType> = (values) => {
        storeRequest({
            data: values
        }, true).then((result) => {
            if (result.status) {
    ...
    const onNextHandler: SubmitHandler<OffRoadForRentType> = (values) => {
                    ...
                    <FormRow required={true} label={"Car Type"} xs={12}>
                        <Controller
                            ...
                            render={({field: {ref, ...field}}) => (
                                <TextField
                                    placeholder={"The type/model name of the car. E.g: F-150, Hilux"}
                                    label={"Car Type"}
                                    inputRef={ref}
...
export default CarOffRoadForRent