...
const AutomobileForSaleSchema = z.object({
    ...
    plateCondition: z.enum(["With Plate Number", "Without Plate Number"], {invalid_type_error: "Should be either With Plate Number or Without Plate Number"}),
...
type AutomobileForSaleType = z.infer<typeof AutomobileForSaleSchema>;

const CarAutomobileForSale = () => {
    ...
    const {
        ...
        handleSubmit,
        control,
    } = useForm<AutomobileForSaleType>({
        resolver: zodResolver(AutomobileForSaleSchema),
    ...
    const onSubmitHandler: SubmitHandler<AutomobileForSaleType> = (values) => {
        storeRequest({
            data: values
        }, true).then((result) => {
            if (result.status) {
    ...
    const onNextHandler: SubmitHandler<AutomobileForSaleType> = (values) => {
...
export default CarAutomobileForSale