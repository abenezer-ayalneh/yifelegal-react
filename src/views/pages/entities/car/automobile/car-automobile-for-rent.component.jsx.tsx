...
const AutomobileForRentSchema = z.object({
...
type AutomobileForRentType = z.infer<typeof AutomobileForRentSchema>;

const CarAutomobileForRent = () => {
  ...
  const {
    ...
    handleSubmit,
    control,
  } = useForm<AutomobileForRentType>({
    resolver: zodResolver(AutomobileForRentSchema),
  ...
  const onSubmitHandler: SubmitHandler<AutomobileForRentType> = (values) => {
    storeRequest(
      {
        data: values,
      },
  ...
  const onNextHandler: SubmitHandler<AutomobileForRentType> = (values) => {
          ...
          <FormRow required={true} label={"Car Type"} xs={12}>
            <Controller
              ...
              render={({ field: { ref, ...field } }) => (
                <TextField
                  placeholder={
                    "The type/model name of the car. E.g: Dezire, Yaris"
                  }
                  label={"Car Type"}
                  inputRef={ref}
            ...
            <Controller
              ...
              render={({ field: { ref, ...field } }) => (
                <TextField
                  type={"number"}
                  placeholder={
                    "How long you are planning on using the car (in days)"
                  }
...
export default CarAutomobileForRent;
