...
type DealPagePropTypes = {
    remove?: "sale" | "rent"
}

const DealPage = ({remove}: DealPagePropTypes) => {
    ...
    return (
        <Box>
            ...
            <Grid container paddingX={{xs: 5, md: 5, lg: 3, xl: 15}} justifyContent={{xs: "left", sm: "center", lg: "left"}} spacing={{xs: 1, sm: 2, md: 3}}>
                {remove !== "sale" && <HorizontalTile tileTitle={"For Sale"} image={ForSale} to={"deal/sale"}/>}
                {remove !== "rent" && <HorizontalTile tileTitle={"For Rent"} image={ForRent} to={"deal/rent"}/>}
            </Grid>
        </Box>
    )
}
